...
This standalone Next.js application turns a YouTube URL into a thorough, timestamped, source-grounded historical bibliography. Captions are retrieved locally, isolated Codex CLI OAuth workers curate meaningful multi-word phrases, and resumable job state is stored under `.runtime/jobs`.
...
`bibliographer.config.toml` is project-owned and references `DEFAULT_PROMPT.md`. Candidate extraction uses high-recall reasoning; synthesis, verification, and the video overview use medium reasoning. The contract uses 12,000-character chunks, 16 candidates per chunk, a usual 40-reference limit with a hard 52-reference ceiling for the final five minutes, and a ten-minute processing budget. User-level Codex configuration and API-key environment variables are not used.
...
2. extracts a thorough set of meaningful multi-word phrases from smaller chronological chunks, including defensible current and recent references;
...
Long runs stop safely as `capped` after ten minutes or the configured hit ceiling. The UI preserves partial results, elapsed/ETA counters, and a continuation action that starts at the saved timestamp. Missing `yt-dlp`, `ffmpeg`, or storyboard formats produce warnings without failing the bibliography.

## Output

The results begin with a video preamble covering title, channel, upload/publication date, description-supported primary participants, theme, and summary when established. Each result is presented as a numbered annotated timeline entry with a linked `HH:MM:SS` timestamp, brief evidence, an original-source link when available, concise bibliographer’s analysis, optional description-only speaker attribution and discussion context, and a collapsed audit section containing confidence, source quality, verification notes, and all source links. Results remain in video order and retain lazy storyboard thumbnails linked to the video timestamp. The Markdown code block lives at `/app/jobs/[jobId]/markdown`; the exact file is downloaded from `/api/historical-references/[jobId]/markdown`.
