...
- **Hit**: A deduplicated, source-grounded phrase shown in the results. A run returns at most 40 hits by default. Each hit may include explicit attribution and a short discussion context.
- **Speaker attribution**: A person or role explicitly supported by the transcript near a hit. It is `null` when the captions do not establish who spoke.
- **Discussion context**: One or two short paragraphs describing what the participants were discussing near a hit timestamp. It is transcript-grounded and distinct from historical analysis.
- **Video overview**: A best-effort preamble containing title, channel, upload/publication date, people, theme, and summary. Metadata is sourced from `yt-dlp`; people, theme, and summary are sourced from captions when supported.
...
- The overview and discussion context are evidence-bounded. Missing metadata or uncertain attribution is shown as unavailable rather than guessed.
